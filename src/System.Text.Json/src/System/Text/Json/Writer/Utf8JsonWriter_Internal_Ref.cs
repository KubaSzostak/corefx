...
namespace System.Text.Json
{
    internal class PooledObject
    {
        ...
        public PooledObject(byte[] rentedBuffer, int version)
        {
    ...
    public ref partial struct Utf8JsonWriter_Internal_Ref
    {
        ...
        /// </remarks>
        public Utf8JsonWriter_Internal_Ref(JsonWriterState state = default)
        {
            ...
            _version = 0;
            _pool = new PooledObject(ArrayPool<byte>.Shared.Rent(MinimumBufferSize), _version);
        }
        ...
        public void Flush(Stream stream, bool isFinalBlock = true)
        {
            ...
            stream.Write(_pool._rentedBuffer);
            ClearHelper();
        ...
        public void Flush(IBufferWriter<byte> output, bool isFinalBlock = true)
        {
            ...
            _pool._rentedBuffer.AsSpan(0, _index).CopyTo(buffer);
        ...
        //    await stream.WriteAsync(_pool._rentedBuffer, cancellationToken);
        //    ClearHelper();
        ...
        private void WriteStartMinimized(byte token)
        {
            ...
            _pool._rentedBuffer[_index++] = token;
        }
        ...
        private void WriteNewLine()
        {
            ...
            if (Environment.NewLine.Length == 2)
            {
                ...
                _pool._rentedBuffer[_index++] = JsonConstants.CarriageReturn;
            }
            ...
            _pool._rentedBuffer[_index++] = JsonConstants.LineFeed;
        }
        ...
        {
            int previousSpanLength = _pool._rentedBuffer.Length;
            CheckAndResizeBuffer(_pool._rentedBuffer.Length + DefaultGrowthSize);
            if (_pool._rentedBuffer.Length <= previousSpanLength)
            {
        ...
        {
            Debug.Assert(_pool._rentedBuffer != null);
            Debug.Assert(sizeHint >= 0);
            Debug.Assert(sizeHint == DefaultGrowthSize);

            int availableSpace = _pool._rentedBuffer.Length - _index;

            if (sizeHint > availableSpace)
            {
                int growBy = Math.Max(sizeHint, _pool._rentedBuffer.Length);

                int newSize = checked(_pool._rentedBuffer.Length + growBy);

                byte[] oldBuffer = _pool._rentedBuffer;

                _pool._rentedBuffer = ArrayPool<byte>.Shared.Rent(newSize);

                Debug.Assert(oldBuffer.Length >= _index);
                Debug.Assert(_pool._rentedBuffer.Length >= _index);

                Span<byte> previousBuffer = oldBuffer.AsSpan(0, _index);
                previousBuffer.CopyTo(_pool._rentedBuffer);
            ...
            Debug.Assert(_pool._rentedBuffer.Length - _index > 0);
            Debug.Assert(_pool._rentedBuffer.Length - _index >= sizeHint);
        }

        private void GrowAndEnsure(int minimumSize)
        {
            CheckAndResizeBuffer(minimumSize);
            if (_pool._rentedBuffer.Length < minimumSize)
            {
        ...
        private void CopyLoop(ReadOnlySpan<byte> span)
        {
            while (true)
            {
                if (span.Length <= _pool._rentedBuffer.Length - _index)
                {
                    span.CopyTo(_pool._rentedBuffer.AsSpan(_index));
                    _index += span.Length;
                    break;
                }

                span.Slice(0, _pool._rentedBuffer.Length - _index).CopyTo(_pool._rentedBuffer.AsSpan(_index));
                span = span.Slice(_pool._rentedBuffer.Length - _index);
                _index = _pool._rentedBuffer.Length;
                GrowAndEnsure();
            }
        }
        ...
        public void Dispose()
        {
            ...
            if (_pool._rentedBuffer == null)
            {
                return;
            }

            ClearHelper();
            ArrayPool<byte>.Shared.Return(_pool._rentedBuffer);
            _pool._rentedBuffer = null;
        ...
        {
            Debug.Assert(_pool._rentedBuffer != null);
            _pool._rentedBuffer.AsSpan(0, _index).Clear();
            _index = 0;
        }
