...
namespace System.Text.Json
{
    public ref partial struct Utf8JsonWriter_Internal_Ref
    {
        ...
        {
            if (!Utf8Formatter.TryFormat(value, _pool._rentedBuffer.AsSpan(_index), out int bytesWritten))
            {
                GrowAndEnsure(_index + JsonConstants.MaximumFormatInt64Length);
                bool result = Utf8Formatter.TryFormat(value, _pool._rentedBuffer.AsSpan(_index), out bytesWritten);
                Debug.Assert(result);
            }
            _index += bytesWritten;
        }
