...
namespace System.Text.Json
{
    public partial class Utf8JsonWriter_Fetch
    {
        ...
        private void WriteNumberMinimized(ReadOnlySpan<char> escapedPropertyName, long value)
        {
            ...
            int idx = WritePropertyNameMinimized(ref buffer, escapedPropertyName);
        ...
        {
            Span<byte> buffer = _output.GetSpan();
            int idx = WritePropertyNameIndented(ref buffer, escapedPropertyName);
        ...
        private void WriteNumberValueFormatLoop(ref Span<byte> buffer, long value, ref int idx)
        {
            if (!Utf8Formatter.TryFormat(value, buffer.Slice(idx), out int bytesWritten))
            {
                AdvanceAndGrow(ref buffer, ref idx, JsonConstants.MaximumFormatInt64Length);
                bool result = Utf8Formatter.TryFormat(value, buffer, out bytesWritten);
                Debug.Assert(result);
            }
            idx += bytesWritten;
        }
