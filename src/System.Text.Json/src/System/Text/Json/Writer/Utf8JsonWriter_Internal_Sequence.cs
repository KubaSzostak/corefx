...
namespace System.Text.Json
{
    ...
    public partial class Utf8JsonWriter_Internal_Sequence : IDisposable
    {
        ...
        {
            private Utf8JsonWriter_Internal_Sequence _this;

            public static SingleThread Lock(Utf8JsonWriter_Internal_Sequence writer)
            {
        ...
        /// </remarks>
        public Utf8JsonWriter_Internal_Sequence(JsonWriterState state = default)
        {
            _buffers = new List<byte[]>();
            _indices = new List<int>();
            _rentedBuffer = ArrayPool<byte>.Shared.Rent(MinimumBufferSize);
            _index = 0;
            _buffers.Add(_rentedBuffer);
            _indices.Add(_index);
            _totalWritten = 0;
            _segment = 0;
        ...
        public void Flush(Stream stream, bool isFinalBlock = true)
        {
            ...
            for (int i = 0; i < _segment; i++)
            {
                ...
                Span<byte> writtenSpan = _buffers[i].AsSpan(0, index);
        ...
        public void Flush(IBufferWriter<byte> output, bool isFinalBlock = true)
        {
            ...
            int total = checked((int)_totalWritten + _index);
            ...
            for (int i = 0; i < _segment; i++)
            {
                int index = _indices[i];
                Span<byte> writtenSpan = _buffers[i].AsSpan(0, index);
                writtenSpan.CopyTo(buffer.Slice(_written));
                writtenSpan.Clear();
                _written += index;
                _indices[i] = 0;
            }
            ...
            //if (_index > 0)
            //{
            //    throw new Exception($"{_totalWritten}, {_index},{_rentedBuffer.Length}, {_segment}, {_written}, {_buffers.Count}, {_indices[_segment]},{_buffers[_segment].Length} ");
            //}

            Span<byte> lastSpan = _rentedBuffer.AsSpan(0, _index);
            lastSpan.CopyTo(buffer.Slice(_written));
            lastSpan.Clear();
            _indices[_segment] = 0;
        ...
        public async Task FlushAsync(Stream stream, CancellationToken cancellationToken = default, bool isFinalBlock = true)
        {
            ...
            {
                int index = _indices[i];
                await stream.WriteAsync(_buffers[i], 0, index, cancellationToken);
                _buffers[i].AsSpan(0, index).Clear();
                _indices[i] = 0;
            }
        ...
        {
            Debug.Assert(_buffers[_segment] == _rentedBuffer);

            _indices[_segment] = _index;
            checked
            {
                _totalWritten += _index;
            }

            CheckAndResizeBuffer(DefaultGrowthSize);
        }
        ...
        private void CheckAndResizeBuffer(int sizeHint)
        {
            ...
            Debug.Assert(_buffers.Count > 0);
            Debug.Assert(_indices.Count == _buffers.Count);
            Debug.Assert(_indices[_indices.Count - 1] == _index && _buffers[_buffers.Count - 1] == _rentedBuffer);
            ...
            if (sizeHint > availableSpace)
            {
                _index = 0;
                _segment++;
                if (_buffers.Count <= _segment)
                {
                    if (sizeHint <= MinimumBufferSize)
                    {
                        _rentedBuffer = ArrayPool<byte>.Shared.Rent(MinimumBufferSize);
                    }
                    else
                ...
                {
                    _rentedBuffer = _buffers[_segment];
                    _indices[_segment] = 0;
                }
            }
        ...
        public void Dispose()
        {
            ...
            foreach (byte[] segment in _buffers)
            {
        ...
        private void ClearHelper()
        {
            ...
            {
                byte[] segment = _buffers[i];
                int index = _indices[i];
                segment.AsSpan(0, index).Clear();
                _indices[i] = 0;
            }
