...
namespace System.Text.Json
{
    public partial class Utf8JsonWriter_Final
    {
        ...
        {
            if (!Utf8Formatter.TryFormat(value, _buffer.AsSpan(_buffered), out int bytesWritten))
            {
                GrowAndEnsure();
                bool result = Utf8Formatter.TryFormat(value, _buffer.AsSpan(_buffered), out bytesWritten);
                Debug.Assert(result);
            }
