...
namespace System.Text.Json
{
    public partial class Utf8JsonWriter_Mix
    {
        ...
        private void WriteStringMinimized(ReadOnlySpan<char> escapedValue)
        {
            ...
            Span<byte> output = GetSpanValue(maxLengthRequired, escapedValue.Length);
            int idx = _buffered;
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private int WriteStringMinimized(ReadOnlySpan<byte> escapedValue, int maxLengthRequired)
        {
            throw new NotImplementedException();
        }
        ...
        private Span<byte> GetSpanValue(int maxLengthRequired, int escapedPropertyNameLength)
        {
            if (_output != null)
            {
                if (_buffer.Length - _buffered < maxLengthRequired)
                {
                    int minLengthRequired = escapedPropertyNameLength + 2;
