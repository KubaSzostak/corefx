...
namespace System.Text.Json
{
    ...
    public partial class Utf8JsonWriter_Memory_TryGet
    {
        ...
        /// </remarks>
        public Utf8JsonWriter_Memory_TryGet(IBufferWriter<byte> bufferWriter, JsonWriterState state = default)
        {
            ...
            _isArrayBacked = MemoryMarshal.TryGetArray(_buffer, out _segment);
        }
        ...
        private void WriteStartMinimized(byte token)
        {
            ...
            if (output.Length - _buffered < 2)
            {
                //GrowAndEnsure(1, 2);
                output = GrowAndEnsureRequired(2);
            }
        ...
        {
            Span<byte> output = _segment.AsSpan();
            if (output.Length - _buffered < 1)
            {
                output = GrowAndEnsureRequired(1);
            }
            output[_buffered++] = token;
            //Advance(1);
        }
        ...
        private void WriteNewLine(ref int idx)
        {
            ...
            if (Environment.NewLine.Length == 2)
            {
                ...
                output[idx++] = JsonConstants.CarriageReturn;
            }
            ...
            output[idx++] = JsonConstants.LineFeed;
        }
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private Span<byte> GrowAndEnsureRequired(int requiredLength)
        {
            if (_buffer.Length - _buffered < requiredLength)
            {
                GrowAndEnsure();
                if (_isArrayBacked)
                {
                    //throw new Exception($"{_buffer.Length}, {_buffered}, {_segment.Count}, {_segment.Offset}, {_isArrayBacked}, {requiredLength}.");
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        private Span<byte> GrowAndEnsureRequired(int minimumSizeRequired, int maximumSizeRequired)
        {
            if (_buffer.Length - _buffered < maximumSizeRequired)
            {
                GrowAndEnsure(minimumSizeRequired, maximumSizeRequired);
                if (_isArrayBacked)
                {
                    return _segment.AsSpan();
                }
