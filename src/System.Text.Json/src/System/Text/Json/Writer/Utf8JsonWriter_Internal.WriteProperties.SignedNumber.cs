...
{
    public partial class Utf8JsonWriter_Internal
    {
        /// <summary>
        ...
        {
            WritePropertyNameMinimized(escapedPropertyName);
        ...
        {
            WritePropertyNameIndented(escapedPropertyName);
        ...
        private void WriteNumberValueFormatLoop(long value)
        {
            if (!Utf8Formatter.TryFormat(value, _rentedBuffer.AsSpan(_index), out int bytesWritten))
            {
                GrowAndEnsure(_index + JsonConstants.MaximumFormatInt64Length);
                bool result = Utf8Formatter.TryFormat(value, _rentedBuffer.AsSpan(_index), out bytesWritten);
                Debug.Assert(result);
            }
            _index += bytesWritten;
        }
