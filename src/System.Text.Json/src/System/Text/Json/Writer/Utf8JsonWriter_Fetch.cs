...
namespace System.Text.Json
{
    ...
    public partial class Utf8JsonWriter_Fetch
    {
        ...
        /// </remarks>
        public Utf8JsonWriter_Fetch(IBufferWriter<byte> bufferWriter, JsonWriterState state = default)
        {
        ...
        private void WriteStartByOptions(ReadOnlySpan<byte> utf8PropertyName, byte token)
        {
            ...
            if (_writerOptions.Indented)
            {
                idx = WritePropertyNameIndented(ref buffer, utf8PropertyName);
            }
            else
            {
                idx = WritePropertyNameMinimized(ref buffer, utf8PropertyName);
            }

            if (1 > buffer.Length - idx)
            {
                AdvanceAndGrow(ref buffer, ref idx, 1);
            }
        ...
        private void WriteStartByOptions(ReadOnlySpan<char> propertyName, byte token)
        {
            ...
            if (_writerOptions.Indented)
            {
                idx = WritePropertyNameIndented(ref buffer, propertyName);
            }
            else
            {
                idx = WritePropertyNameMinimized(ref buffer, propertyName);
            }
        ...
        private void WriteNewLine(ref Span<byte> buffer, ref int idx)
        {
            // Write '\r\n' OR '\n', depending on OS
        ...
        private void GrowAndEnsure(ref Span<byte> buffer)
        {
            int previousSpanLength = buffer.Length;
            Debug.Assert(previousSpanLength < DefaultGrowthSize);
            buffer = _output.GetSpan(DefaultGrowthSize);
            if (buffer.Length <= previousSpanLength)
            {
                ThrowHelper.ThrowArgumentException(ExceptionResource.FailedToGetLargerSpan);
            }
        }

        private void GrowAndEnsure(ref Span<byte> buffer, int minimumSize)
        {
        ...
        private void AdvanceAndGrow(ref Span<byte> buffer, ref int alreadyWritten)
        {
            Debug.Assert(alreadyWritten >= 0);
            _output.Advance(alreadyWritten);
            GrowAndEnsure(ref buffer);
            alreadyWritten = 0;
        }

        private void AdvanceAndGrow(ref Span<byte> buffer, ref int alreadyWritten, int minimumSize)
        {
            Debug.Assert(minimumSize >= 1 && minimumSize <= 128);
            _output.Advance(alreadyWritten);
            GrowAndEnsure(ref buffer, minimumSize);
            alreadyWritten = 0;
        }

        private void CopyLoop(ref Span<byte> buffer, ReadOnlySpan<byte> span, ref int idx)
        {
            while (true)
            {
                if (span.Length <= buffer.Length - idx)
                {
                    span.CopyTo(buffer.Slice(idx));
                    idx += span.Length;
                    break;
                }

                span.Slice(0, buffer.Length - idx).CopyTo(buffer.Slice(idx));
                span = span.Slice(buffer.Length - idx);
                idx = buffer.Length;
                AdvanceAndGrow(ref buffer, ref idx);
            }
        }
